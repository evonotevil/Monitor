...
_REGION_GROUP_MAP = {
    ...
    # 大洋洲
    "大洋洲": "大洋洲", "澳大利亚": "大洋洲", "新西兰": "大洋洲",
    # 其他
    "全球": "其他", "其他": "其他",
}

_GROUP_ORDER = ["东南亚", "南亚", "中东", "欧洲", "北美", "南美", "日韩台", "大洋洲", "其他"]
...
_TEXT_GROUP_PATTERNS = [
    ...
    ("南亚",   r'(?i)\b(india[n]?|dpdpa\b|meity\b|vaishnaw|pakistan|bangladesh)\b'),
    ("日韩台", r'(?i)\b(japan[ese]?|korea[n]?|south korea|grac\b|kca\b|cero\b'
               r'|nintendo\b|台湾|韓[国國]?)\b|日本|ゲーム|게임|확률형'),
    ...
    ("大洋洲", r'(?i)\b(australia[n]?|new zealand|esafety\b|accc\b)\b'),
]
...
def _dedup_for_display(items: List[dict]) -> List[dict]:
    """
    报告渲染前内容去重：
    - 同一显示分组内，中文标题（或原标题）bigram 相似度 > 60% 的条目视为同一事件
    - 优先保留 impact_score 最高 + source_tier 最权威的条目
    - 被合并的条目数量追加在保留条目的摘要末尾
    """
    ...
    for idx in sorted_idx:
        ...
        for kidx in kept_idx:
            ...
            if _bigram_sim(t_item, t_kept) > 0.60:
...
(function() {{
  ...
  // 按预设顺序填充地区下拉
  const groupOrder = ["东南亚","南亚","中东","欧洲","北美","南美","日韩台","大洋洲","其他"];
