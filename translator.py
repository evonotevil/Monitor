...
越南 印度尼西亚 泰国 菲律宾 马来西亚 新加坡 澳大利亚 新西兰
印度 巴基斯坦 孟加拉国 斯里兰卡
台湾 香港 澳门 日本 韩国
沙特 阿联酋 土耳其 尼日利亚 南非
全球 其他
...
摘要规则（summary_zh 三段式结构，总计 60-90 字）：
- 内容必须与标题显著不同，严禁复制或改写标题
- 必须包含以下三个部分，每部分约 20-30 字（移动端优先）：
  【核心整改点】明确指出需要改动的功能/条款（如：移动端 IAP 抽成规则、Gacha 概率公示、IDFA 采集同意机制、年龄验证 SDK）
  【多端业务影响】⚠️ 必须先写移动端（App Store/IAP/Google Play），再写 PC 端（Steam/D2C/Anti-cheat）；禁止颠倒顺序；若仅影响单端则注明
  【行动建议】基于监管紧迫度给出操作指令（优先级：紧急/高/中）。禁止提及任何具体部门名称（如"技术部""运营部"）；直接说明需要关注或完成的事项（如：需在 30 天内完成移动端 SDK 升级，同步评估 PC 官网支付页合规性）
- 示例句式："该政策将直接影响移动端 IAP 抽成，同时需关注其对 PC 官网充值页面的反向穿透影响"
...
def generate_executive_summary(items: list) -> str:
    ...
    # 取 impact_score 最高的 20 条做素材（避免超 token）
    sorted_items = sorted(items, key=lambda x: float(x.get("impact_score", 0)), reverse=True)[:20]
    ...
    material = "\n".join(material_lines)
    user_msg = (
        f"以下是本期全球游戏行业立法监控动态（共 {len(items)} 条），"
        f"请以合规分析师视角，为 Lilith 等中资出海游戏公司撰写一段不超过 300 字的中文月报综述。\n"
        f"要求：\n"
        f"1. 点出 2-3 个最值得关注的监管趋势或高风险事件\n"
        f"2. 明确指出哪些市场/政策对移动端（App Store/Google Play）影响最大\n"
        f"3. 给出简要行动提示（关注什么，优先级如何）\n"
        f"4. 语言简洁专业，直接输出综述正文，不含标题、序号或 Markdown\n\n"
        f"动态素材：\n{material}"
    )
