...
_TERM_CORRECTIONS: dict[str, str] = {
    ...
    # PC 平台专有名词
    "启动器": "Launcher",               # 防止"启动器"被意译（保留英文更准确）
    "应用内购买": "IAP",                 # In-App Purchase
    "应用内购": "IAP",
    "抗作弊": "Anti-cheat", "反作弊程序": "Anti-cheat",
    "充值": "Top-up",                   # 第三方充值语境下保留英文
}
...
# 纯别名键（分组名 / 泛称），不作为 LLM 可选的单一国家值放入 Prompt 列表，
# 但仍保留在 _VALID_REGIONS 中以接受 LLM 偶发输出。
_REGION_PROMPT_SKIP = {
    "北美", "欧洲", "东南亚", "亚太", "亚太区",
    "港澳", "港澳台", "日韩", "日韩台",
    "南美", "南亚", "非洲", "大洋洲", "中东", "中东/非洲", "独联体",
}


def _build_region_prompt_section() -> str:
...
_AI_SYSTEM = (
    """你是全球游戏合规高级分析师，专注于 Hoyoverse、Lilith Games、鹰角等中资游戏公司的出海合规监控。
战略定位：移动端（App Store/Google Play/IAP）是核心营收渠道，须最高优先级；PC端（Steam/Epic/D2C/Anti-cheat）是高利润增量，次高优先级。
处理游戏监管新闻，需完成四项任务并输出 JSON。

⚠️ 非英语内容处理原则（关键）：
若标题/摘要为日文、韩文、越南文、泰文等非英语，必须先内部理解/翻译，再进行分类和摘要生成。
严禁仅因无法识别非英语文字而判定为"不相关"——日韩越等地区的游戏监管动态同样重要。

【任务一：相关性判断（is_relevant）】
is_relevant = false 的情形——直接返回 {"is_relevant": false}，无需翻译：
- 游戏评测 / 新作发布 / DLC / 赛季更新 / Patch Notes / 游戏攻略
- 电竞赛事 / 赌场博彩 / 体育博彩 / 彩票
- 纯技术工具更新（SDK / IDE / API 版本发布，无监管背景）
- 公司财报 / 融资 / 裁员（无监管动作）
- 排行榜 / 最佳游戏推荐 / 开发者大会/峰会活动
- 硬件/设备评测（处理器架构、Wi-Fi标准、电池优化、GPU/CPU性能测试）
- 仅涉及 Google/Apple 产品新闻但与支付/分发/分级/隐私/消费者保护无关（如 Pixel/iPhone 评测、MacBook 发布、季报营收）
⚠️ 原则：进入此流程的文章已通过关键词预筛，大多数有实质监管意义。只有明确识别为上述噪音时才返回 false，边界情形默认判定 true。
is_relevant = true：涉及立法/监管/执法/政策草案/合规要求的游戏行业动态，包括：
- 移动端（App Store/Google Play 政策、IAP、IDFA/GAID、SDK 合规、第三方支付）— 最高优先级
- PC端（Steam/Epic/Launcher/Anti-cheat/D2C）— 次高优先级
- Gacha/Loot Box、未成年人保护、数据隐私、跨境数据合规、地区经营许可

【任务二：地区识别（region）】
从文章内容判断最相关地区，从以下选一个：
"""
    + _build_region_prompt_section()
    + """
⚠️ FTC、DOJ、CISA、CPPA、CCPA、COPPA 等美国机构/法案 → 填"美国"；GDPR、DSA、DMA → 填"欧盟"；港交所、PCPD、SFC → 填"香港"。
若参考地区已给出且合理，优先采用；若不准确则修正。

【任务三：分类与状态】
category_l1 选一个：数据隐私 玩法合规 未成年人保护 广告营销合规 消费者保护 经营合规 平台政策 内容监管 PC & 跨平台合规

分类优先级规则（有歧义时按此判断）：
- "平台政策"适用于：App Store/Google Play 政策变更、IAP 规则、第三方支付强制要求（DMA）、IDFA/GAID 隐私政策、平台审核/下架规定
  ↳ 与"消费者保护"冲突时：若法规主体是平台（苹果/谷歌/Steam），选"平台政策"；若主体是政府针对商家/玩家，选"消费者保护"
  ↳ 与"经营合规"冲突时：若核心是平台准入/分成规则，选"平台政策"；若核心是本地营业执照/税务/代表处，选"经营合规"
- "未成年人保护"适用于：年龄验证、未成年消费限额、游戏时长管控、家长控制、COPPA/儿童隐私
  ↳ 与"内容监管"冲突时：若焦点是"谁能玩/花多少钱"，选"未成年人保护"；若焦点是"内容本身是否合法/分级"，选"内容监管"
- "数据隐私"适用于：用户数据收集/传输/本地化、隐私政策合规、执法罚款——即使涉及儿童数据，只要核心是数据处理，仍选"数据隐私"
- "PC & 跨平台合规"适用于：PC Launcher 权限、Anti-cheat 程序合规、PC 端 D2C/Top-up、跨平台账号数据传输
...
法规/机制：GDPR、COPPA、CCPA、DSA、DMA、PDPA、LGPD、Loot Box、Gacha、NFT、DLC、IAP、Anti-cheat、Launcher、Top-up、IDFA、GAID
技术术语：Deepfake、App Store、Google Play、SDK（AI 可译为"人工智能"）
...
摘要规则（summary_zh，总计 60-90 字）：
- 用流畅自然的专业中文写成，像资深合规顾问写给业务负责人的一段话，禁止模板化套语
- 内容必须与标题显著不同，严禁复制或改写标题
- 必须涵盖以下要素（自然融入行文，不要用标签/分段）：
  ① 监管核心要求——具体规则/条款/禁止事项是什么；若有明确截止日期或生效时间，必须写出
  ② 业务影响——优先说明对移动端（App Store/IAP/Google Play）的影响，若同时影响 PC 端（Steam/D2C/Anti-cheat）则续写，若仅影响单端则只写该端
  ③ 具体行动——指出需要做什么，如"须更新隐私政策条款""须在 X 日前完成年龄验证接入""须下架不合规 IAP 产品"；禁止使用"紧急/高优/中优"等空洞定性词，禁止提及具体部门名称
- 若原始内容极少，须基于专业知识合理扩充；禁止简单复述标题
...
def _ai_process_batch(items_data: list) -> list:
    ...
    for i, d in enumerate(items_data):
        ...
        lang_hint = ""
        if d.get("lang") and d["lang"].split("-")[0] in {"ja", "ko", "vi", "th", "id", "zh"}:
            lang_note = {"ja": "日文", "ko": "韩文", "vi": "越南文", "th": "泰文", "id": "印尼文", "zh": "中文"}.get(d["lang"].split("-")[0], "")
            if lang_note:
                lang_hint = f"\n语言提示：本条目为{lang_note}，请先理解再分类，不得因语言陌生判定不相关。"

        parts.append(
            f"【文章{i + 1}】\n"
            f"标题（英/日/韩/越等）：{title_val}\n"
            f"原始摘要：{d.get('summary', '') or '（无）'}"
            f"{hint_line}{lean_warn}{lang_hint}"
        )
...
def generate_executive_summary(items: list) -> str:
    """
    将所有条目摘要拼接后，调用一次 LLM 生成不超过 300 字的中文月报综述。
    展示在 HTML 报告最顶部，帮助读者快速把握本期核心监管动态。

    返回生成的综述文本，失败时返回空字符串。
    items: list of dict，每条含 title_zh / summary_zh / region / category_l1 字段。
    """
    if not items:
        return ""
    if not (_HAS_AI and _AI_CLIENT):
    ...
    # 全量扫描：所有条目均纳入素材（按 impact_score 降序排列，让模型优先感知高权重条目）
    ...
    material = "\n".join(material_lines)
    user_msg = (
        f"以下是上周全球游戏行业立法监控动态（共 {len(items)} 条，已按重要性排序），"
        f"请以资深合规分析师视角，为 Lilith 等中资出海游戏公司撰写一段 200-300 字的中文综述。\n\n"
        f"必须满足以下结构（自然融入行文，不要用序号或标签分段）：\n"
        f"① 上周最大的 2 个风险点——具体说明是哪个市场/政策/监管动作，对移动端或 PC 端的实质威胁\n"
    ...
    try:
        resp = _AI_CLIENT.chat.completions.create(
            ...
            extra_body=_LLM_EXTRA_BODY,
            messages=[
                {"role": "system", "content": "你是资深游戏行业合规分析师，专注中资出海游戏公司监管风险研究。"},
                {"role": "user",   "content": user_msg},
            ],
        )
        text = resp.choices[0].message.content.strip()
        ...
        if len(text) > 320:
            # 找最后一个句号截断
            cut = text[:320].rfind("。")
            text = text[:cut + 1] if cut > 0 else text[:300]
        logger.info(f"[综述] 生成成功，{len(text)} 字")
        return text
    except Exception as e:
        logger.warning(f"[综述] LLM 生成失败: {e}")
        return ""


# ── 周报飞书卡片摘要（A+B 关键词 + 风险提示，100 字以内）─────────────

def generate_weekly_card_summary(news_items: list) -> str:
    """
    ...
    sorted_items = sorted(news_items, key=lambda x: float(x.get("impact_score", 0)), reverse=True)

    material_lines = []
    for it in sorted_items[:20]:   # 最多取 20 条，避免 token 过长
        title   = (it.get("title_zh") or it.get("title") or "").strip()
        summary = (it.get("summary_zh") or "").strip()
        region  = (it.get("region") or "").strip()
        cat     = (it.get("category_l1") or "").strip()
        if title:
            line = f"[{region}/{cat}] {title}"
            if summary:
                line += f"：{summary[:80]}"
            material_lines.append(line)
    ...
    try:
        ...
        text = resp.choices[0].message.content.strip()
        logger.info(f"[周报卡片摘要] 生成成功，{len(text)} 字")
        return text
    except Exception as e:
        logger.warning(f"[周报卡片摘要] LLM 生成失败: {e}")
        return ""
