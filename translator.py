...
_AI_SYSTEM = _load_prompt("ai_system.txt").replace(
    "{region_list}", _build_region_prompt_section()
)
_PROMPT_VERIFY_DUP = _load_prompt("verify_duplicate.txt")
_PROMPT_MERGE_DUP = _load_prompt("merge_duplicate.txt")
_PROMPT_EXEC_SUMMARY = _load_prompt("executive_summary.txt")
_PROMPT_WEEKLY_CARD = _load_prompt("weekly_card_summary.txt")
_PROMPT_DAILY_SUMMARY = _load_prompt("daily_summary.txt")
...
def verify_duplicate_pairs(pairs: list) -> list:
    ...
    try:
        resp = _AI_CLIENT.chat.completions.create(
            ...
            extra_body=_LLM_EXTRA_BODY,
            messages=[
                {"role": "system", "content": _PROMPT_VERIFY_DUP},
                {"role": "user", "content": user_msg},
            ],
        )
        text = resp.choices[0].message.content.strip()
...
def merge_duplicate_summaries(primary: dict, duplicates: list) -> str:
    ...
    try:
        resp = _AI_CLIENT.chat.completions.create(
            ...
            extra_body=_LLM_EXTRA_BODY,
            messages=[
                {"role": "system", "content": _PROMPT_MERGE_DUP},
                {"role": "user",   "content": user_msg},
            ],
        )
...
def generate_executive_summary(items: list) -> str:
    ...
    try:
        resp = _AI_CLIENT.chat.completions.create(
            ...
            extra_body=_LLM_EXTRA_BODY,
            messages=[
                {"role": "system", "content": _PROMPT_EXEC_SUMMARY},
                {"role": "user",   "content": user_msg},
            ],
        )
        text = resp.choices[0].message.content.strip()
...
def generate_weekly_card_summary(news_items: list) -> str:
    ...
    try:
        resp = _AI_CLIENT.chat.completions.create(
            ...
            extra_body=_LLM_EXTRA_BODY,
            messages=[
                {"role": "system", "content": _PROMPT_WEEKLY_CARD},
                {"role": "user",   "content": user_msg},
            ],
        )
        text = resp.choices[0].message.content.strip()
...
def generate_daily_summary(items: list) -> str:
    ...
    try:
        resp = _AI_CLIENT.chat.completions.create(
            ...
            extra_body=_LLM_EXTRA_BODY,
            messages=[
                {"role": "system", "content": _PROMPT_DAILY_SUMMARY},
                {"role": "user",   "content": user_msg},
            ],
        )
        text = resp.choices[0].message.content.strip()
