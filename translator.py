...
- 必须包含以下三个部分，每部分约 20-30 字（移动端优先）：
  ...
  【多端业务影响】先阐述移动端影响（App Store/IAP/Google Play），再说明 PC 端（Steam/D2C/Anti-cheat）的反向穿透影响；若仅影响单端则注明
  【行动建议】基于监管紧迫度给出操作指令（优先级：紧急/高/中；如：建议技术部 30 天内完成移动端 SDK 升级，同步评估 PC 官网支付页合规性）
