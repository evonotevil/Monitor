...
def _dashboard_cta_elements(dashboard_url: str) -> list:
    """构建 Card 2.0 看板入口；空地址时完全省略。"""
...
def _build_card_v2(
    *,
    ...
    """Build the shared Card 2.0 shell used by every daily status."""
    return {
        "schema": "2.0",
        "config": {
            "update_multi": True,
            "width_mode": "default",
            "summary": {"content": f"{title} · {subtitle}"},
        },
        "header": {
...
def build_daily_card(
    ...
    return _build_card_v2(
        title="全球游戏合规动态",
        subtitle=f"日报 · {date_label}",
        template=header_color,
        icon_token="calendar_outlined",
        elements=elements,
    )
...
def main():
    ...
    is_monday = weekday == 0
    dashboard_url = os.environ.get("DAILY_DASHBOARD_URL", "").strip()
    ...
    if not push_items:
        ...
        empty_elements.extend(_dashboard_cta_elements(dashboard_url))
        empty_card = _build_card_v2(
            title="全球游戏合规动态",
            subtitle=f"日报 · {date_label}",
            template="green",
            icon_token="calendar_outlined",
